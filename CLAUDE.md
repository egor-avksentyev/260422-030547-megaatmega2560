...
`dimmerMenuIndex()`/`colorMenuIndex()` (`main.h`/`main.cpp`) — поиск индекса "Dimmer"/"Color" в `menuItems[]` по имени (по образцу существующего `volumeMenuIndex()`), нужны там, где `currentMenuItem` не гарантированно указывает на нужный пункт — например в `on_off_logic.cpp` при сохранении/загрузке EEPROM.
...
**EEPROM** (только в `on_off_logic.cpp`, через `<EEPROM.h>`) — три независимых слота по разным адресам, чтобы не пересекаться:
...
- `EEPROM_DIMMER_COLOR_ADDR` (16) — `struct SavedDimmerColorSettings { uint16_t magic; uint8_t ringDimmerPercent; uint8_t displayBrightnessPercent; uint8_t ringColorIndex; }`, магическое число `0xD1C0`. В отличие от двух слотов выше, **не привязан к выключению питания** — пишется через `saveDimmerColorSettings()` сразу при любом изменении яркости колец (LED), яркости дисплея (Display) или цвета колец (Color), с энкодера или с пульта (см. "Dimmer" выше). Загружается через `loadDimmerColorSettings()`, вызывается из `loadSettings()` (`main.cpp`) в самом начале `setup()` — раньше, чем `applyDisplayBrightness()`/`applyRingColorScheme()` в первый раз применят значения по умолчанию.

Все три слота пишутся через `EEPROM.put()`, который сам, побайтно, сравнивает с уже записанным значением и не трогает физическую EEPROM, если ничего не изменилось (снижает износ ячеек — актуально и для частых циклов включения/выключения без реального изменения положения ручек, и для частого кручения Dimmer/Color, которое пишет в EEPROM на каждый шаг).
