...
// Пауза (мс) перед подачей нового направления, если motorControl()/motorControl2() увидели
// смену знака (мотор крутился в одну сторону, команда пришла в другую) — резкий мгновенный
// реверс H-моста под нагрузкой даёт бросок тока (обратная ЭДС обмотки), который проседает
// общую линию питания и на мгновение сбивает WS2812 (нет восстановления тайминга из линии
// данных — просадка/скачок VCC искажает бит, а ошибка сдвигает все последующие, поэтому
// обычно "мигает" последнее кольцо в цепочке — Volume). На это время мотор просто стоит
// (coast, не активное торможение) — токи успевают спасть перед разворотом. Подбирается по
// ощущениям на реальном железе: слишком мало — брос всё ещё виден, слишком много — заметная
// заминка при быстром вращении туда-сюда
#define MOTOR_REVERSE_BRAKE_MS 30
...
// На сколько мс после команды НАВИГАЦИИ по меню (Right/Left/Enter/Set — см.
// lastMenuNavigationTime(), remote_control.h) придерживаем обновление колец выше. Кольца в
// этот момент никого не интересуют (пользователь смотрит на экран меню, не на кольца), а вот
// NeoPixel.show() внутри renderDbRing()/updateVolumeRing() отключает прерывания на время
// бит-банга — если это окно накладывается на приём очередного RC5-кадра, кадр пульта
// теряется (тот же механизм, что уже решён для Up/Down через throttledSliderRedraw() в
// remote_control.cpp, здесь — то же самое для навигации). НЕ применяется к Up/Down — там
// кольца должны жить в реальном времени, пока крутишь Bass/High/Volume
#define MENU_NAVIGATION_RING_PAUSE_MS 250
// Раз в сколько мс обновляется экран Info (три температуры DS18B20), пока он открыт — датчику
// нужно время на конверсию (см. readAllTemperatures() в temperature_sensor.cpp, конверсия не
// блокирует loop() — просто читает результат прошлого запроса и тут же просит следующий), так
// что даже с запасом чаще 750-1000мс смысла нет: температура физически не меняется быстрее
#define INFO_UPDATE_INTERVAL_MS 1000
...
// --- Датчики температуры DS18B20, пункт меню "Info" (см. temperature_sensor.h/.cpp) ---
// Все три на ОДНОЙ 1-Wire шине (один пин, один резистор-подтяжка) — изначально были на трёх
// отдельных шинах (по резистору на каждую), но переехали на общую из-за нехватки резисторов
// под рукой. DallasTemperature сам перечисляет несколько датчиков на одной шине по индексу
// (getTempCByIndex(0/1/2)) — вручную прописывать 64-битные ROM-адреса не нужно. Порядок
// индексов — это порядок обнаружения при сканировании шины (не привязан к тому, в каком
// физическом месте припаян датчик) — если после прошивки показания перепутались местами,
// поменяй местами подписи в tempSensorLabels[] ниже, а не провода
#define TEMP_SENSOR_PIN A14
// Подписи к показаниям на экране Info, по порядку обнаружения на шине — поправь под то, что
...
// Угол стрелки (от вертикали, см. drawArrowIndicator() в display_logic.cpp) на самом
// краю шкалы (valueMin/valueMax) — общий для Bass/High/Volume, одна и та же функция
// отрисовки на все три. Раньше было ровно 120° — крайние положения смотрелись как будто
// стрелка ещё не докрутилась до упора. +10° сверху делает крайний завод заметнее
#define ARROW_NEEDLE_MAX_ANGLE_DEG 140
// Угол ровно на середине пути от центра шкалы (0dB/50%) к краю — должен быть строго
// горизонтальным (90°). Для Bass/High это ±5дБ (половина от ±10дБ), для Volume — 25%/75%
// (половина пути от 50% к 0%/100%); при текущей калибровке оба случая — это ровно половина
// диапазона от центра до края, так что drawArrowIndicator() считает угол одной и той же
// формулой на все три ручки без хардкода конкретных "5"/"25"
#define ARROW_NEEDLE_HORIZONTAL_ANGLE_DEG 90
