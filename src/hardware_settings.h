...
#define SLIDER_MOTOR_SPEED 15 // Фиксированная скорость моторов Bass/High/Volume при вращении энкодера/пульта (диапазон -50..50). Было 40, потом 25 — понижено по просьбе, общее для энкодера/пульта/автовозврата
...
#define MOTOR_ZERO_TIMEOUT_MS 20000 // Максимум ждём при выключении, пока Bass/High/Volume доедут до нуля —
// предохранитель на случай упора/заклинившего потенциометра, чтобы не блокировать выключение навечно.
// Было 3000, потом 6000 — хватало только на "типичный" случай (Volume около
// VOLUME_POWERON_TARGET_PERCENT=30% или ниже) при прежней SLIDER_MOTOR_SPEED=40. При включённой
// на полную громкости (100%) моторы Bass/High/Volume едут к нулю все вместе на одном общем
// таймауте (seekBassHighVolumeToZeroBlocking()), а полный ход Volume 100%->0% физически занимает
// заметно больше времени, чем короткий ход от 30% — таймаут истекал раньше, чем ручка Volume
// реально доезжала до нуля, и stopAllMotors() глушил мотор на середине пути. Пересчитано (6000
// -> 20000) после понижения SLIDER_MOTOR_SPEED с 40 до 15 — тот же физический путь на медленной
// скорости занимает пропорционально больше времени (примерно ×2.7), плюс запас, т.к. на низком
// PWM момент мотора ближе к порогу старта под нагрузкой и путь может занять непропорционально
// больше "чистой" линейной оценки. Если ручка всё ещё не успевает доехать от 100% при
// выключении — увеличивай дальше по тому же принципу (пропорционально текущей SLIDER_MOTOR_SPEED)
