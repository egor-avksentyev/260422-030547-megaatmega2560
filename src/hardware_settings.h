...
// --- Полноэкранный показ положения ручки (Bass/High/Volume) при обнаружении ручного
// вращения РУКОЙ, пока сидим на карусели меню — см. блок обнаружения в main.cpp, loop(),
// и drawArrowIndicator() (display_logic.cpp), тот же экран, что и настоящий вход в
// настройки. Раньше здесь был компактный OLED-only вариант (drawKnobPositionOverlay()) —
// полноэкранный тогда не использовали, т.к. он дёргает NeoPixel.show() через
// renderDbRing()/updateVolumeRing(), а движение рукой никак не коррелирует с приёмом ИК
// (в отличие от команд навигации, для которых была своя пауза) — с переходом на ICU-декодер
// (rc5_icu.h) эта коллизия устранена на уровне приёма, компактный вариант убран за
// ненадобностью ---

// Обнаружение ручного вращения сравнивает УЖЕ ПЕРЕВЕДЁННОЕ значение (dB для Bass/High, %
// для Volume), а НЕ raw ADC — потенциометры откалиброваны сильно неравномерно
// (bassPotCalRaw[]/highPotCalRaw[]/volumePotCalRaw[] ниже), и порог в raw-отсчётах почти
// недостижим при медленном вращении у самого начала шкалы (-10dB у Bass/High, 0% у Volume),
// где raw меняется очень мало на градус поворота — там же, где обычно начинают крутить,
// если ручка была на нуле.
//
// Сравнение — окно целиком (значение KNOB_OVERLAY_CONSECUTIVE_TICKS тиков назад против
// текущего, не с зафиксированной раз и навсегда точкой — иначе после реального перемещения
// ручки в новое положение сравнение никогда не осядет обратно, и индикатор завис бы навсегда;
// и не по одному соседнему тику за раз — у Bass/High всего ~21 грубая ступенька дБ на весь
// диапазон, и соседние тики иногда не отличаются вообще даже во время настоящего вращения).
// См. sustainedKnobMovement() в main.cpp за деталями окна.
//
// Порог свой у Bass/High (в дБ) и у Volume (в %) — Volume ловит дребезг контакта потенциометра
// (особенно в нижней части хода — см. VOLUME_RING_FIRST_LED_*_PERCENT ниже, та же наводка),
// который может дать скачок значения на 1-3% сам по себе, без прикосновения к ручке. Порог
// Bass/High шире делать не нужно — там задача была в разрешении (см. выше), а не в наводке
#define KNOB_OVERLAY_VALUE_THRESHOLD_DB 1
#define KNOB_OVERLAY_VALUE_THRESHOLD_PERCENT 5
#define KNOB_OVERLAY_CONSECUTIVE_TICKS 3
// Через сколько мс без нового движения руки возвращаемся в карусель
#define KNOB_OVERLAY_IDLE_TIMEOUT_MS 2000
// Порог KNOB_OVERLAY_VALUE_THRESHOLD_DB настолько низкий (1 единица), что после реального
// вождения мотора (Up/Down с пульта/энкодера, requestXSeek()) остаточное механическое
// "доседание" редуктора/потенциометра за несколько сотен мс после остановки мотора уже
// может быть принято за настоящее ручное вращение — и сразу после отпускания Up/Down (или
// выхода из настроек) экран показывался бы ПОВТОРНО, как будто рукой. Поэтому обнаружение
// не срабатывает не только пока мы не на карусели, но и ещё KNOB_OVERLAY_MOTOR_SETTLE_MS
// после последней команды НА МОТОР (lastMotorInputTime, main.h) — не важно, откуда она
// пришла (энкодер/пульт/автовозврат)
#define KNOB_OVERLAY_MOTOR_SETTLE_MS 500
//
...
// Скорость ТОЛЬКО прямого ручного управления — вращение энкодера и Up/Down с пульта/веб-
// страницы (та делит код с пультом, см. executeMenuCommand() в remote_control.cpp). Диапазон
// -50..50. Было 40, потом 25, сейчас 15 — понижено по просьбе именно для ручного вращения
#define SLIDER_MOTOR_SPEED 15
// Скорость ВСЕХ автоматических/фоновых переходов — автовозврат после Bypass, восстановление
// сохранённой позиции при включении питания, доезд к цели EQ-пресета (все три —
// updateBassHighRecenter()/updateVolumeSeek(), motor_driver_logic.cpp), и доезд в ноль при
// выключении (seekBassHighVolumeToZeroBlocking()). Раньше всё это тоже ехало на
// SLIDER_MOTOR_SPEED — после того как ту понизили ради ручного вращения, стали заметны
// побочные эффекты в местах, где скорость не должна была меняться (например Volume не
// дотягивала до истинного нуля за прежний по времени "слепой" доворот при выключении).
// Разделили: у ручного вращения своя скорость (SLIDER_MOTOR_SPEED), у всего автоматического —
// снова быстро, как было изначально
#define MOTOR_SEEK_SPEED 40
...
// 100%->0% на скорости MOTOR_SEEK_SPEED физически занимает заметно больше времени, чем короткий
// ход от 30% — таймаут истекал раньше, чем ручка Volume реально доезжала до нуля, и
// stopAllMotors() глушил мотор на середине пути (~50%). Значение здесь привязано к
// MOTOR_SEEK_SPEED (автоматические переезды) — не к SLIDER_MOTOR_SPEED (ручное вращение, на
// доезд в ноль при выключении не влияет). Если после прошивки ручка всё ещё не успевает
// доехать от 100% при выключении — увеличивай дальше по тому же принципу
...
// --- ИК-приёмник и коды команд пульта. После переобучения пульт шлёт все кнопки
// одним протоколом RC5, Address=0x18 — сюда записан только Command, коллизий
// между значениями нет (полная таблица — IR_REMOTE_CODES.md) ---
//
// Декодирование — собственный RC5-декодер на Input Capture Timer4 (rc5_icu.h/.cpp),
// не библиотека IRremote (от неё отказались — таймерный опрос старого IR_PIN=19
// периодически терял кадры, если приём совпадал по времени с NeoPixel.show(),
// который отключает прерывания на время бит-банга колец; см. подробный разбор в
// rc5_icu.h). ICP4 = Arduino-пин 49 (PL0) — само число здесь не используется в
...
// сырой протокол на выделенном проводе, без совместимости с чем-либо ещё, можно выбирать
// скорость свободно
#define FRAME_MIRROR_BAUD 500000
// Не чаще, чем это, даже если экран перерисовывается заметно чаще (например каждые 200мс при
// вращении Bass/High/Volume) — при FRAME_MIRROR_BAUD=500000 один кадр (1027 байт с чексуммой)
// блокирует loop() примерно на ~20мс каждый раз, когда реально отправляется
#define FRAME_MIRROR_MIN_INTERVAL_MS 150
...
// Скорость этого доворота — MOTOR_SEEK_SPEED (автоматический переезд, не ручное вращение)
