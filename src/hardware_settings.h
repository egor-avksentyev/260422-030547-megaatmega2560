...
// Значение specifically НЕ равно 200мс (внутренний антидребезг Right/Left/Enter/Set в
// remote_control.cpp) — при удержании кнопки реально принятые нажатия идут с интервалом
// ближе к ~228мс (114мс repeat-кадр RC5, первый повтор ещё внутри 200мс-окна и гасится,
// второй уже проходит), плюс джиттер/изредка целиком потерянный кадр — тот же расчёт, что
// уже когда-то стоил проекту нескольких итераций подбора SLIDER_MOTOR_IDLE_TIMEOUT (260мс).
// Если это окно слишком короткое, пауза колец успевает закончиться МЕЖДУ двумя реально
// принятыми нажатиями при удержании — и именно в этот момент могут вернуться подтупливания.
// Даём заметный запас сверху вместо повторной подгонки по миллиметру
#define MENU_NAVIGATION_RING_PAUSE_MS 500
